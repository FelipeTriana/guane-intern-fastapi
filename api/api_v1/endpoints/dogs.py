...
@router.get("/dogs/is_adopted/", response_model=List[schemas.Dog])
...
@router.post("/dogs/{user_id}/", response_model=schemas.Dog)
def create_dog_for_user(user_id: int, dog: schemas.DogCreate, db: Session = Depends(deps.get_db )):
    return crud.create_user_dog(db=db, dog=dog, user_id=user_id)

@router.put("/dogs/{dog_id}/", response_model=schemas.Dog)  #Hay que agregar excepciones
def update_dog(dog_id: int, dog_update: schemas.DogUpdate, db: Session = Depends(deps.get_db )):
    return crud.update_dog(db=db, dog_update=dog_update, dog_id=dog_id)

@router.delete("/dogs/{dog_id}", response_model=schemas.Del)
